...
class NotificationService {
  ...
  static async showImmediateNotification(title: string, body: string, channelId: string = CHANNEL_ID_ACHIEVEMENT) {
  ...
  static async rescheduleAllNotifications() {
    ...
    if (active.weeklyInsightsEnabled) {
      await this.scheduleWeeklyInsights(active.weeklyInsightsDay, active.weeklyInsightsTime);
    }
  ...
  static async cancelAllScheduled() {
    const ids = [NOTIF_ID_DAILY, NOTIF_ID_STREAK, NOTIF_ID_BREAK, NOTIF_ID_WEEKLY, NOTIF_ID_INACTIVITY];
    for (const id of ids) {
